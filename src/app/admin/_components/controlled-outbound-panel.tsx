...
export function ControlledOutboundPanel({ onAction }: { onAction: AdminAction }) {
  ...
  if (loading && !data) return <Panel title="Apollo Manual & Tautan Kampanye"><p className="text-sm text-slate-500">Memuat kontrol outbound manual…</p></Panel>;
  if (!data?.phase20Part2Ready) return <Panel title="Apollo Manual & Tautan Kampanye"><p className="text-sm text-amber-800">Phase 20.2 belum tersedia di database. Terapkan migrasi 0050 sebelum membuat tautan UAT.</p></Panel>;

  return <section className="space-y-4" aria-labelledby="controlled-outbound-heading">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#D9A441]">Phase 20.2</p><h2 id="controlled-outbound-heading" className="mt-1 text-lg font-bold text-[#0B2C6B]">Apollo Manual & Konversi Kampanye</h2></div><button type="button" onClick={() => void load()} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold"><RefreshCw size={14} className={loading ? "animate-spin" : ""} /> Perbarui</button></div>
    <p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-950"><strong>Kontrol aman:</strong> panel ini tidak mengirim email dan tidak memanggil Apollo API. Ia hanya membuat satu tautan UAT opaque untuk prospek yang sudah lolos validation, suppression, source, campaign, dan approval. Pengiriman manual ke kontak eksternal tetap di luar Phase 20.2.</p>
    ...
    <Panel title="Buat satu tautan UAT" action="Tidak mengirim email">
    ...
    <Panel title="Audit tautan terbaru" action={`${data.links.length} tautan`}><div className="overflow-x-auto"><table className="w-full min-w-[600px] text-left text-xs"><thead><tr className="border-b text-[10px] uppercase tracking-wide text-slate-400"><th className="pb-3">Kampanye</th><th className="pb-3">Mode</th><th className="pb-3">Klik</th><th className="pb-3">Berlaku sampai</th></tr></thead><tbody>{data.links.slice(0, 12).map((link) => { const campaign = data.campaigns.find((item) => item.id === link.campaign_id); return <tr key={link.id} className="border-b border-slate-100"><td className="py-3 font-semibold text-[#0B2C6B]">{campaign?.name || "Kampanye"}</td><td>UAT manual · {link.status}</td><td>{clicksByLink[link.id] || 0}</td><td>{new Date(link.expires_at).toLocaleString("id-ID", { dateStyle: "medium", timeStyle: "short" })}</td></tr>; })}</tbody></table>{!data.links.length && <p className="py-5 text-sm text-slate-500">Belum ada tautan UAT.</p>}</div><p className="mt-4 text-xs text-slate-500">{data.apolloPro?.message}</p></Panel>
  </section>;
}
