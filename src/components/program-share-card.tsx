    ...
    <section className="rounded-2xl border border-[#0B2C6B]/10 bg-white p-5 text-left shadow-[0_20px_60px_-48px_rgba(11,44,107,0.55)] sm:p-6" aria-labelledby="share-program-title">
      ...
      <div className="mt-4 space-y-3">
        <div className="rounded-xl bg-[#F5F7FA] p-3">
          <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.16em] text-[#4A4C54]/55"><Link2 className="h-3.5 w-3.5" /> Tautan program</div>
          <p className="mt-1.5 break-all text-xs font-semibold text-[#0B2C6B]">{link}</p>
          <button type="button" onClick={() => void copy(link, "Tautan program disalin.")} className="mt-2 inline-flex min-h-9 items-center gap-2 rounded-lg border border-[#0B2C6B]/15 bg-white px-3 text-xs font-bold text-[#0B2C6B]"><Copy className="h-3.5 w-3.5" /> Salin tautan</button>
        </div>
